...
**FID ≈ 20.3**. A WGAN-GP baseline trained on the same architecture reaches
FID ≈ 16.9 — better on FID, but this framework matches it almost exactly on
pixel-space W2 distance (the metric it's actually optimizing) despite
solving the harder, unrelaxed transport problem.
...
problems. This was the single biggest FID improvement in the whole MNIST
study: FID drops to **≈ 15.1**, with **98%+ classification accuracy** on
generated digits and better manifold coverage than the WGAN-GP baseline at
...
embeddings via Adaptive Group Normalization (AdaGN). There's no established
FID benchmark for this exact setup, so evaluation here is qualitative:
...
- **`optimizers.py`** — the update rules: `SGD_update`, `adam_update`,
  ...
  identical across all three experiments — one implementation, reused
  everywhere.
...
- **`metrics.py`** — training diagnostics (gradient-norm variants used to
  ...
  in the thesis's quantitative MNIST tables — live in the *validation*
  notebook, not yet migrated into this structure (see "What's not here
  yet" below).
...
- **`MNIST_Experiment.ipynb`** — the largest notebook, organized into
  ...
  **Model CW** (the WGAN-GP baseline), followed by a later addendum of
  ...
  left active. **Worth knowing:** the conditional models the thesis reports
  numbers for (C1/C2) don't have an active training cell in the source
...
- **CartoonSet training needs a GPU; the other three notebooks don't.**
  2D, MNIST, and CartoonSet preprocessing all run fine on CPU (slowly, for
  MNIST/CartoonSet preprocessing).
